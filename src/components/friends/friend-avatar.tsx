...
export function FriendAvatar({ friend, size = "md" }: { friend: VrchatUser; size?: "sm" | "md" }) {
    ...
    return (
        <span className={`relative inline-flex shrink-0 ${sizeClass}`}>
            ...
            <span className="absolute right-0 bottom-0 size-2.5 rounded-full border-2 border-card" style={{ backgroundColor: statusColor(friend.status) }} aria-hidden="true" />
        </span>
    );
}
