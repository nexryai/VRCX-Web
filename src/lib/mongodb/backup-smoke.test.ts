    ...
    test("compares every document, collection option, and relevant index definition", async () => {
        ...
        try {
            ...
            for (const database of [source, restored]) {
                ...
                const activityEvents = database.collection<BackupActivityDocument>("activity_events");
                await activityEvents.insertMany([
                ...
                await activityEvents.createIndex({ ownerId: 1, occurredAt: -1 }, { name: "owner_occurred", partialFilterExpression: { ownerId: { $type: "string" } } });
            ...
            await restored.collection<BackupActivityDocument>("activity_events").updateOne({ _id: "second" }, { $set: { "payload.bytes": Buffer.from([9]) } });
