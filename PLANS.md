...
User results preserve the avatar, display name, trust level, languages, and one-line bio structure and open the maintained User Dialog. World results preserve VRCX's compact `180px` card grid and open the maintained World Dialog. Group results preserve its dense avatar/name/member/short-code/description rows and continue to use safe VRChat web links until a maintained Group Dialog is ported. Narrow screens reflow the tab/search toolbar and grid without changing the desktop information hierarchy.
