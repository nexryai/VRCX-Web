...
User-specific persistent-data state follows `misc.js`, `worldCoordinator.js`, and the same World Dialog command. Migration 39 adds owner/world-unique `world_persist_snapshots`. Opening a World Dialog asks the server to reconcile the fixed `users/{activeUserId}/{worldId}/persist/exists` resource; VRChat's 404 response is recorded as the authoritative false state, and a transient failure may return the last snapshot marked stale. Delete Persistent Data appears for both owned and non-owned worlds only when that projection is true, calls only the fixed active-user/world child, and atomically projects false after upstream success. This per-world capability has no enumerable realtime source, so reconciliation is server-side and on demand when a concrete World is inspected rather than a browser-owned durable state or a fabricated global scan. Download Unity Package remains excluded because it is a local package workflow.
...
`VRCX/src/views/Settings/Settings.vue`, `components/SettingsGroup.vue`, `components/SettingsItem.vue`, `components/Tabs/SystemTab.vue`, `components/Tabs/InterfaceTab.vue`, and `components/Tabs/SocialTab.vue` define the settings page hierarchy and dense card rows. The web port exposes the working System, Interface, and Social categories, VRCX-style underlined tabs and groups, application/version/legal links, theme, navigation collapse, favorite ordering, the current per-workflow table page sizes, and the friend Favorite Groups Filter. Changes write through the validated singleton MongoDB settings boundary and update the live shell where applicable.

The Social favorite filter follows `stores/settings/general.js`, `stores/friend.js`, and `coordinators/favoriteCoordinator.js`. Migration 40 adds the bounded, canonical `localFavoriteFriendsGroups` setting. A selected `friend:<VRChat group name>` key limits remote friend favorites to those groups; when no remote group key is selected all remote friend favorites are included, matching VRCX. MongoDB-local friend favorites remain included regardless of the remote-group filter. The owner-scoped `/api/friend-favorites` projection applies that rule once on the server and is shared by Feed, Friend List, Friends Locations, and the friends sidebar, so local favorites and groups beyond the first upstream page are not silently omitted. Local selector values use the stable MongoDB group ID instead of VRCX's rename-sensitive local group name; stale or foreign-owner IDs select no data and never cross the active-owner boundary.

Settings export produces a versioned JSON download containing only validated browser-safe application preferences. Import validates the exact format and rejects unknown or server-owned fields before replacing those preferences in MongoDB. VRChat cookies, active identity state, MongoDB configuration, monitor leases, histories, caches, and secrets are never included. Favorite ID/CSV transfer remains a separate, working Favorites workflow and is not mislabeled as part of this preferences backup. The current VRCX Settings and My Avatars sources expose avatar-tag editing but no avatar-tag import/export command, so a new transfer surface is not treated as a parity requirement.
...
VRCX's Windows startup, tray, GPU, updater, local proxy, VR, OpenVR, media/filesystem, local TTS condition, Discord/game-process integration, registry, and similar controls are absent instead of disabled because their behavior depends on the desktop or local VRChat. Eligible browser notification delivery and the recent-action indicator remain future work and will be added only with their working browser/server behavior; empty tabs are not shown. The earlier Manage-menu link to the About placeholder has been replaced by the maintained `/settings` route, while the complete copied license remains available from its System legal section.
...
The 2026-08-23 World Dialog persistent-data increment passed `pnpm test` (42 files, 188 tests), `pnpm lint` (240 files), and `pnpm build` on Next.js 16.3.0; inventories now cover 42 GET handlers and 45 mutation handlers across 37 mutation route files, and the generated route manifest contains the fixed persistence endpoint. Migration 39 and MongoDB integration verify owner/world uniqueness, indexes, replacement, and owner isolation; allowlist coverage rejects persistence subpaths other than the exact resource and `/exists`. Owned and non-owned Delete Persistent Data confirmations passed registered 360- and 1280-pixel captures; all four screenshots were manually inspected, while Playwright verified capability-gated menu access, Cancel initial focus, focus trapping, Escape, and Manage-trigger focus restoration. Authenticated live existence/404/deletion behavior and a matched running-VRCX comparison remain open.

The 2026-08-23 Settings Social favorite-group increment passed `pnpm test` (43 files, 191 tests), `pnpm lint` (245 files), and `pnpm build` on Next.js 16.3.0; the cache-policy inventory now covers 43 GET handlers and the generated route manifest contains the owner-scoped favorite-friend projection. Schema coverage verifies canonical remote/local group keys, bounds and uniqueness. Migration 40 and MongoDB integration verify default backfill, selected remote-group filtering, unconditional local-favorite inclusion and active-owner isolation. The Social tab and its populated group selector passed registered 360- and 1280-pixel captures without page-level horizontal overflow; all four screenshots were manually inspected, while Playwright verified the remote/local grouping, Escape dismissal and trigger-focus restoration. Authenticated live group updates and a matched running-VRCX comparison remain open.
