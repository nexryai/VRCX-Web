...
The owned-world Manage menu follows `WorldDialog.vue` and `useWorldDialogCommands.js`. Rename, description, hard capacity, recommended capacity, and YouTube preview edits are available only when a freshly fetched authoritative world identifies the active singleton identity as its author. Each same-origin request accepts exactly one maintained field, uses the fixed `worlds/{worldId}` request, enforces the resulting recommended-capacity invariant, updates the owner-scoped MongoDB world projection, and preserves upstream-success semantics when the local projection needs reconciliation. The browser accepts the same direct YouTube ID and common URL workflows, while the server persists only a validated compact video ID. VRCX's world tag, allowed-domain, image, publish/unpublish, persistent-data, and delete actions remain eligible follow-up work. Download Unity Package remains excluded because it is a local package workflow.
